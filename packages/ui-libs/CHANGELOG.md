...
- **Row Type Export**: Added `Row` type export from `@tanstack/react-table` in `table-elements` subpath
  - Users can now import `Row` type directly: `import type { Row } from 'tanstack-shadcn-table/table-elements'`
  - Makes it easier to type custom row actions and handlers without importing from `@tanstack/react-table` directly
